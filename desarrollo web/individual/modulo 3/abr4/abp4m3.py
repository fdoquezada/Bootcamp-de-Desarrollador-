...
usuarios = ['fernando', 'andres', 'agustin', 'benjamin', 'hector',
           'manuel ', 'carlos']
encuestas = ['habilidades alimenticios','empleavilidad','experiencia  laboral','actividad recreativa', ]

inicio = time.time()
           
"""for sorteo in range(7):
    tiradas= usuarios[random.randint(0, 6)]
    encuestas = encuestas[random.choise]
    
    
    time.sleep(0.5)

    print('Encuestas', sorteo , encuestas + 1, ':', tiradas)"""
for sorteo in range(7):
        tiradas= usuarios[random.randint(0, 6)]
encuestas = random.sample(encuestas,4)
